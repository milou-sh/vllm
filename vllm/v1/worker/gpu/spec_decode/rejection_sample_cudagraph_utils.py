...
def _copy_inputs_with_padding_kernel(
    ...
    if pid < num_reqs:
        ...
        tl.store(dst_top_p_ptr + idx, tl.load(src_top_p_ptr + idx))
...
class RejectionSamplerCudaGraphManager:
    def __init__(
        ...
        max_num_reqs: int,
        num_speculative_steps: int,
        vocab_parallel_nucleus: bool = False,
    ):
        ...
        self.vocab_parallel_nucleus = vocab_parallel_nucleus
    ...
    def can_run(
        ...
        if has_grammar or not self.has_graph(desc):
            return False

        assert self.rejection_sampler is not None
        if self.vocab_parallel_nucleus:
            return (
                ...
                and self.rejection_sampler.can_vocab_parallel_nucleus(
                    input_batch, self.draft_logits
                )
            )

        sampler = self.rejection_sampler.sampler
        idx = input_batch.idx_mapping_np
        states = sampler.sampling_states
        return (
            input_batch.num_draft_tokens > 0
            and not input_batch.is_prefilling_np.any()
            and states.max_num_logprobs(idx) == NO_LOGPROBS
            and sampler.logprob_token_ids_state.max_num_token_ids(idx) == 0
            and not (states.top_k.np[idx] != states.vocab_size).any()
            and (states.top_p.np[idx] != 1.0).any()
            and not sampler.penalties_state.use_penalty[idx].any()
            and sampler.bad_words_state.num_bad_words.np[idx].max() == 0
            and not sampler.logit_bias_state.use_logit_bias[idx].any()
        )
    ...
    def capture(
        ...
        compute_logits_fn: Callable[
            [torch.Tensor], torch.Tensor | tuple[torch.Tensor, int]
        ],
    ...
    def _init_persistent_buffers_for_capture(
        ...
        self.top_p.fill_(0.95 if self.vocab_parallel_nucleus else 1.0)
        self.seeds.fill_(0)
    ...
    def _sample(
        ...
        logits_output = compute_logits_fn(sample_hidden_states)

        if self.vocab_parallel_nucleus:
            assert isinstance(logits_output, tuple)
            local_logits, vocab_start = logits_output
            sampler_output = rejection_sampler.forward_vocab_parallel_graph(
                local_logits,
                vocab_start,
            ...
            sampler_output = rejection_sampler.forward_graph(
                logits_output,
                input_buffers.input_ids,
                input_buffers.positions,
                logits_indices,
    ...
    def run(
        ...
        _copy_inputs_with_padding(
            ...
            self.sampling_states.top_p.gpu,
